...
static public class NGUIText
{
	...
	static public void Update (bool request)
	{
		...
		if (dynamicFont != null && request)
		{
			...
			if (dynamicFont != null
				&& (!dynamicFont.GetCharacterInfo(')', out mTempChar, finalSize, fontStyle) || mTempChar.vert.height == 0f))
			{
	...
	{
		bool layout = false;
		return WrapText(text, out finalText, false, ref layout);
	}
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, ref bool needRelayout)
	{
		...
		bool fits = true;
		bool eastern = false;
		bool isArabic = I2.Loc.LocalizationManager.CurrentLanguageCode == "ar";
		needRelayout = false;
...
//			D.warn("asdfwef " + finalText);
	...
	static public bool IsOneLine (string text)
	{
		int prev = 0;
		float glyphWidth = 0f;
		float remainingWidth = regionWidth;
