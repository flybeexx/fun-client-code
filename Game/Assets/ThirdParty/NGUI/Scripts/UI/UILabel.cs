	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				// Wrap the text
				bool layout = false;
				bool fits = NGUIText.WrapText(mText, out mProcessedText, true, ref layout);
				if(layout)
				{
				...
				{
                    if(layout && mProcessedText.Length > 1)
                    {
                        mProcessedText = mProcessedText.Substring(0, mProcessedText.Length - 1);
                    }
					mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText);
				}
